  ...
  {:else}
    <div style="display: flex; flex-direction: column; gap: 8px;">
      {#each data.leaderboard as entry, i}
        <div style="display: flex; align-items: center; gap: 12px; background: {i === 0 ? 'rgba(201,168,76,0.06)' : 'var(--bg-card)'}; border: 1px solid {i === 0 ? 'rgba(201,168,76,0.2)' : 'var(--border)'}; border-radius: 8px; padding: 12px 16px; {i === 0 ? 'box-shadow: 0 0 16px rgba(201,168,76,0.1);' : ''}">
          <!-- Position -->
          <div style="width: 28px; text-align: center; font-weight: 700; font-size: 14px; {i === 0 ? 'color: var(--gold);' : i === 1 ? 'color: #a0a0a0;' : i === 2 ? 'color: #b87333;' : 'color: var(--text-dim);'}">
            {i < 3 ? ['🥇','🥈','🥉'][i] : `${i + 1}`}
          </div>
          <!-- Avatar -->
          ...
          <div style="flex: 1; min-width: 0;">
            <div style="font-size: 13px; font-weight: 600; {i === 0 ? 'color: var(--gold);' : ''}">{entry.display_name}{entry.label ? ` (${entry.label})` : ''}</div>

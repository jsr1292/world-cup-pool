<script lang="ts">
  ...
  type M = { home_code: string; home_score: number; away_code: string; away_score: number; minute: string };
  ...
  <div class="live-ticker" role="status" aria-label="Marcadores en directo">
    ...
    <div class="live-track-wrap">
      <div class="live-track" class:scroll={matches.length > 1}>
        {#each matches as m}
          <span class="live-item"><b>{m.home_code}</b> <span class="sc">{m.home_score}–{m.away_score}</span> <b>{m.away_code}</b>{#if m.minute}<span class="min"> {m.minute}</span>{/if}</span>
        {/each}
        {#if matches.length > 1}
          {#each matches as m}
            <span class="live-item" aria-hidden="true"><b>{m.home_code}</b> <span class="sc">{m.home_score}–{m.away_score}</span> <b>{m.away_code}</b>{#if m.minute}<span class="min"> {m.minute}</span>{/if}</span>
          {/each}
        {/if}
      </div>
    </div>
